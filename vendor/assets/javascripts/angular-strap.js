    ...
    link: function postLink(scope, element, attrs) {
      ...
      if(!attrs.bsAlert) {
        ...
        scope.$watch(attrs.bsAlert, function(newValue, oldValue) {
          ...
          if(closeAfter > 0) {
            // console.log('setting timeout on alert', element);
            $timeout(function(){
              element.alert('close');
            }, closeAfter);
          }
...
.directive('bsDropdown', ['$parse', '$compile', function($parse, $compile) {
  ...
  '<ul class="dropdown-menu" role="menu" aria-labelledby="drop1">' +
    '<li ng-repeat="item in items" ng-class="{divider: !!item.divider, \'dropdown-submenu\': !!item.submenu && item.submenu.length}">' +
      '<a ng-hide="!!item.divider" tabindex="-1" ng-href="{{item.href}}" ng-click="{{item.click}}" target="{{item.target}}" ng-bind-html-unsafe="item.text" rel="nofollow" data-method="{{item.method || \'get\'}}"></a>' +
    '</li>' +
  '</ul>';
